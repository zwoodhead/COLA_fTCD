...
root_dir<-("/Users/zoe/OneDrive - Nexus365/COLA_fTCD_data/") # Set data directory path here
meansdir<- paste0(root_dir, "Analysis/Subject_Means/")
if(!file.exists(meansdir)){dir.create(meansdir)}
plotdir <- paste0(root_dir, "Analysis/LI_Plots/")
...
filelist_loc <- paste0(root_dir,site,"/fTCD_data_",site,".csv")
...
for (mysub in 2){   #Change the numbers here to say which participants you want to analyse. You can just do one subject at once if needed.
  mysubname <- filelist$Gorilla_ID[mysub]
  
  cat(mysubname, "\n\n")
  
  ########################################################
  # Loop through tasks
  mydir <- paste0(root_dir,site,'/',mysubname,'/')
  ...
  # Read exp data
  dataloc <- paste(mydir,"fTCD_",mysubname,"_",task,".exp", sep = "")
...
#    }
    ...
    if (ninclude >= mintrials){ # Continue with analysis if there are enough good trials; otherwise skip it
      ...
      if (initialdatacheck4==1)
      {
        ...
        png(filename=paste0(plotdir,"LI_Plot_",mysubname,"_",task,".png"))
      ...
      if(length(keepmarkers)>=8){
        ...
        csvFile2<-paste(meansdir, mysubname,"_",task,".csv",sep="")
